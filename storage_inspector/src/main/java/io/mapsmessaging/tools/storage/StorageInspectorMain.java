...
public final class StorageInspectorMain {
  ...
  private static final String USAGE = """
      ...
      Usage: StorageInspectorMain --input DIRECTORY [--output events.ndjson]
             ...
             [--destination NAME] [--start-event-id ID] [--end-event-id ID]
      ...
      --destination matches resourceName exactly; event ID bounds are inclusive.
  ...
  private static int scan(Path root, Options options, ReadOnlyStore.Sink sink) throws IOException {
    ...
    Set<Path> directories = java.util.concurrent.ConcurrentHashMap.newKeySet();
    Set<Path> resources = new HashSet<>();
    java.util.Map<Path, Boolean> selected = new java.util.HashMap<>();
    long[] unresolvedMetadata = new long[1];
    boolean[] destinationMatched = new boolean[1];
        ...
        @Override public FileVisitResult preVisitDirectory(Path directory, BasicFileAttributes attrs) throws IOException {
          if (options.destination != null) {
            try {
              ...
              boolean matches = metadata != null && options.destination.equals(metadata.topic());
              selected.put(directory, matches);
              if (metadata != null && !matches) return FileVisitResult.SKIP_SUBTREE;
              if (matches) destinationMatched[0] = true;
            } catch (IOException e) {
          ...
          if (Files.exists(directory.resolve("resource.yaml"), LinkOption.NOFOLLOW_LINKS)) {
            ...
            if (!options.range.filtered()) pool.submit(() -> inspectResource(directory, sink));
          }
          return FileVisitResult.CONTINUE;
        }
        @Override public FileVisitResult visitFile(Path file, BasicFileAttributes attrs) throws IOException {
          if (options.destination != null && !Boolean.TRUE.equals(selected.get(file.getParent()))) return FileVisitResult.CONTINUE;
          String name = file.getFileName().toString();
          if (name.startsWith("partition_") && (!options.range.filtered() || name.matches("partition_[0-9]+_index"))) {
            pool.submit(() -> {
              long[] result = inspectFile(file, root, options, sink);
              if (result[0] > 0) directories.add(file.getParent());
              return result;
            });
          }
          return FileVisitResult.CONTINUE;
    ...
    if (unresolvedMetadata[0] > 0) {
      issue(root, "WARNING", "DESTINATION_SELECTION_INCOMPLETE", unresolvedMetadata[0] + " metadata files could not be read; destination matching is incomplete", sink);
      totals[2]++;
    }
    if (totals[0] == 0) {
      String code = options.destination != null && !destinationMatched[0] ? "NO_MATCHING_DESTINATION" : options.range.filtered() ? "NO_MATCHING_PARTITIONS" : "NO_STORES";
      issue(root, "WARNING", code, "No supported partitions inspected for the requested selection", sink);
      totals[2]++;
    }
    JsonObject summary = ReadOnlyStore.base(root, "summary");
    summary.addProperty("threads", options.threads);
    if (options.destination != null) summary.addProperty("destination", options.destination);
    if (options.range.filtered()) {
      summary.addProperty("scope", "SELECTED_EVENT_IDS");
      summary.addProperty("startEventId", options.range.start());
      summary.addProperty("endEventId", options.range.end());
    }
  ...
  private static long[] inspectFile(Path file, Path root, Options options, ReadOnlyStore.Sink sink) throws Exception {
    ...
    if (name.matches("partition_[0-9]+_index")) {
      ...
      ReadOnlyStore reader = new ReadOnlyStore(options.maxRecordBytes, options.maxArchiveBytes, root, decoder, sink, options.range);
      ReadOnlyStore.Stats result = reader.inspect(file, options.topic);
      if (result.skipped) return totals;
      totals[0]++;
  ...
  private static final class Options {
    ...
    Long startEventId;
    Long endEventId;
    EventRange range = EventRange.ALL;
    ...
    static Options parse(String[] args) {
      ...
      for (int i = 0; i < args.length; i++) {
        ...
        String value = args[i];
        switch (arg) {
          case "--destination" -> options.destination = value;
          case "--start-event-id" -> options.startEventId = Long.parseLong(value);
          case "--end-event-id" -> options.endEventId = Long.parseLong(value);
      ...
      if (options.destination != null && options.destination.isBlank()) throw new IllegalArgumentException("--destination must not be blank");
      options.range = new EventRange(options.startEventId, options.endEventId);
      return options;
    }
  }
}
